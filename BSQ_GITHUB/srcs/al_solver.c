...
/*   Updated: 2024/12/10 22:13:50 by tchockbu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
unsigned int	*obstacle_arr(t_8bit *mp, t_mppt mppt)
{
	...
	//printf("%u * %lu << mppt.obl Malloc\n",mppt.obn, sizeof(unsigned int *));
	//printf("%u << temp\n",temp);
	while (i < temp)
	{
		if (mp[i] == OBSTACLE)
...
void	set_boundary_sl(t_8bit *mp, t_mppt mppt, unsigned int *slr)
{
	...
	if (slr[0] > slr[3])
	{
		...
		slr[5] = mppt.col + 1 - slr[3];
		slr[6] = mppt.row + 1 - slr[3];
		//printf("sqr = %3u lastest boundary : %u %u\n", slr[3], slr[5], slr[6]);
	}
}
...
t_rtm	solver(t_8bit *mp, t_mppt mppt)
{
	...
	while (sl[2] < sl[6])
	{
		...
		while (sl[1] < sl[5])
		{
			...
			//printf("%d ",sl[2] * mppt.col + sl[1]);
			sl[1]++;
		}
		sl[2]++;
	}
	free(mppt.obl);
	//printf("size %u pos = %u/(%u,%u)\n", sl[3], sl[4], sl[4] % mppt.col, sl[4] / mppt.col);
